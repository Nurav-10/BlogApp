...
const CommentSection = ({ postId }: { postId: string }) => {
  ...
  const createReply = async (parentCommentId: string) => {
    if (commentReply.length < 1)
      return toast.success("comment can be of atleast 1 character");
  ...
  const deleteReply=async(commentId:string)=>{
    const response=await fetch(`/api/posts/${postId}/comment/${commentId}/replyDelete`,{
      method:'DELETE'
    })
    const res=await response.json()
  ...
  return (
    <>
      <div className="px-10 flex gap-3 flex-col pb-5">
        ...
        { comments.length===0 && <h2 className="text-lg ml-1">Be the first to Comment</h2>}
        <form
          onSubmit={async (e) => {
            e.preventDefault();
            await createComment();
                ...
                <div key={c._id} className="flex flex-row gap-2">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    {activeReplyIdsIndex.includes(c._id) && (
                      <div className="flex flex-col gap-3 border rounded-md pr-4 pl-2 py-2 border-zinc-500">
